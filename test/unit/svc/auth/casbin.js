...
import newEnforcer from './enforcer.js';
import newAdapter from './adapter.js';
import casbin from './casbin.js';

describe('casbin 接口测试', () => {
  describe('传入管理员标识，为其设置 root 角色', () => {
    it('传入字符串', async () => {
      const admin = 'admin';
      const e = await casbin(Sqlite(':memory:'), admin);
      const ret = await e.enforce(admin, 'foo', 'bar', 'war');
      expect(ret).to.be.true;
    });
    it('传入数字，自动转换为字符串', async () => {
      const admin = 10000;
      const e = await casbin(Sqlite(':memory:'), admin);
      const ret = await e.enforce(admin.toString(), 'foo', 'bar', 'war');
      expect(ret).to.be.true;
    });
  });
